...
public class Search2D : MonoBehaviour
{
    ...
    public Vector2 limitLow = new Vector2(0, 0);        // Search perimeter lower limit
    public Vector2 limitHigh = new Vector2(100, 100);   // Search perimeter upper limit
    public float rayPlaneHeight = 100f;     // Ray max height (max search height)
    public float rayCastDepth = -100f;      // Ray max depth (max search depth)
    ...
    private void OnDrawGizmos() {
        ...
        if (drawSearchArea) {
            ...
            Gizmos.DrawWireCube(new Vector3((limitHigh.x - limitLow.x) / 2 + limitLow.x, rayPlaneHeight, (limitHigh.y - limitLow.y) / 2 + limitLow.y), new Vector3((limitHigh.x - limitLow.x), 0.1f, (limitHigh.y - limitLow.y)));

            Gizmos.color = Color.blue;
            Gizmos.DrawWireCube(new Vector3((limitHigh.x - limitLow.x) / 2 + limitLow.x, rayCastDepth, (limitHigh.y - limitLow.y) / 2 + limitLow.y), new Vector3((limitHigh.x - limitLow.x), 0.1f, (limitHigh.y - limitLow.y)));
        }
